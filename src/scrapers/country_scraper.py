...
def scrape_supported_countries() -> dict[tuple[str, str], str]:
    setup_scraper()
...
def update_supported_countries_json() -> None:
    supported_countries: dict[tuple[str, str], str] = scrape_supported_countries()
...
def read_supported_countries_json() -> dict[tuple[str, ...], str]:
...
def check_country_is_supported(country: str) -> bool:
    supported_countries: dict[tuple[str, ...], str] = read_supported_countries_json()
    is_supported: bool = any(country in key for key in supported_countries.keys())
    if not is_supported:
